<template>
    <div class="content_wrapper md:content_wrapper-md">
      ...
      <div class="hero_info md:hero_info-md">
        <h1 class="hero_title" :style="textStyle('title', data.style)" v-html="data.title"></h1>
        <div class="hero_text" v-html="data.text.content" :style="textStyle('paragraph', data.style)"></div>
        <div class="action_container">
          <a class="action" v-for="item in data.text.buttons" :style="uiStyle('action', data.style)" :href="item.url" :key="item.url" target="_blank">
            {{item.text}}
          </a>
        </div>
      </div>
